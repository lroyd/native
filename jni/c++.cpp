...
JNIEnv *checkCallbackThread (void) 
{
	...
	{
        if(g_sJavaVMIN->AttachCurrentThread (&pEnv, NULL)!= JNI_OK)
		{
...
static int app_callback(int _iEvt, int _iLen, char *_pMessage)
{
	...
	{
		jstring jMsg = pEnv->NewStringUTF(_pMessage);
		pEnv->CallVoidMethod (g_tJniCallbacksObj, method_funcCallback_1, _iEvt, jMsg);
...
static void detach_current_thread (void *_pEnv) 
{
    ...
    if(g_sJavaVMIN->DetachCurrentThread() != JNI_OK)
	{
...
{
    jclass jClazz = _pEnv->FindClass(JAVA_CLASS_NAME);	
	/*********************** #BEGINE get Java callbacks list  ******************/
	method_funcCallback_1 = _pEnv->GetMethodID (jClazz, METHOD_FUNC_CALLBCK_NAME_1, METHOD_FUNC_CALLBCK_SIGN_1); 
...
{
	g_tJniCallbacksObj = _pEnv->NewGlobalRef(_jObject);
#ifdef USER_INTERFACE	
	app_callback(1,2,"init-c++");	//TEST 
...
#endif	
	_pEnv->DeleteGlobalRef(g_tJniCallbacksObj);	
	return 0;
}
...
jint JNI_OnLoad(JavaVM *_pJavaVM, void *_pNULL) 
{
    ...
    if (_pJavaVM->GetEnv((void**) &pEnv, JNI_VERSION_1_4) != JNI_OK) 
	{
        LOG_E("Could not retrieve JNIEnv");
        return 0;
    }

    jclass jClazz = pEnv->FindClass(JAVA_CLASS_NAME);
    pEnv->RegisterNatives (jClazz, g_tNativeMethodsTables, sizeof(g_tNativeMethodsTables)/sizeof(JNINativeMethod));
...
void JNI_OnUnload(JavaVM *_pJavaVM,void *_pNULL) 
{
    ...
    jclass jClazz = pEnv->FindClass(JAVA_CLASS_NAME);
    jint iRet = pEnv->UnregisterNatives(jClazz);
    if(iRet == 0)
